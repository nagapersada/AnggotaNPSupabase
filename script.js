...
async function fetchMembersFromSupabase() {
    try {
        ...
        // Silent error biar ga ganggu kalau cuma masalah koneksi sebentar
        console.error(error); 
        return [];
    }
}
...
async function addMember() {
    ...
    if (error) {
        // Backup untuk jaga-jaga kalau kolomnya huruf besar
...
// --- FUNGSI DIAGRAM JARINGAN (DIKEMBALIKAN KE VERSI ASLI + SATU BARIS) ---
function renderNetwork() {
    const $ = go.GraphObject.make;
    if (diagram) diagram.div = null;
    
    // KEMBALI KE PENGATURAN ASLI (Ada nodeSpacing & contentAlignment)
    ...
    const nodes = membersToRender.map(m => {
        ...
        // FORMAT SATU BARIS (GARIS MIRING)
...
// --- FUNGSI DOWNLOAD (DIKEMBALIKAN KE VERSI ASLI - HIGH QUALITY) ---
function downloadNetworkImage() {
    if (!diagram) { showNotification("Diagram belum dimuat."); return; }

    try {
        // PENGATURAN ASLI: Ada maxSize Infinity agar gambar tidak terpotong
        ...
        link.href = img.src; 
        // Nama file dikembalikan seperti semula
...
// --- FUNGSI LAINNYA (Login, Chart, Table) ---
function login() {
